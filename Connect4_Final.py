...
def ValidatePiece(board, input):       # error checking to ensure a piece can go into the input column
    if type(input) != int:      # if input is not type int
        ...
        return False
    elif input >= 1 and input <= 7 and board[int(input)-1] == "E":   # if the top slot in the selected column is empty then we are good to go
        return True
    else:
        return False
                ...
                for x in range(len(ok_answers)):
                    if player_input == str(ok_answers[x]):
                        ...
                        if ValidatePiece(game_board, final_input) == True:
                            ...
                            message_from_last_player = (f"Player 1 (PLAYER): {player_one_name} plays B in column {final_input}")
                            the_input = True
                ...
                for x in range(len(ok_answers)):
                    if player_input == str(ok_answers[x]):
                        ...
                        if ValidatePiece(game_board, final_input) == True:
                            ...
                            message_from_last_player = (f"Player 2 (PLAYER): {player_two_name} plays W in column {final_input}")
                            the_input = True
...
def AIOnePlay(player_name, player_ID, color):   #Left this as AIOne despite it being logic for all AI since I plan to have several options
    ...
    while the_input == False:
        ...
        if ValidatePiece(game_board, AI_Guess_Random) == True:
            ...
            message_from_last_player = (f"Player {player_ID} (AI): {player_name} plays {color.upper()} in column {AI_Guess_Random}")
            the_input = True

# SuperSmart AI profile. - created to test multiple AIs
def SSAIPlay(player_name, player_ID, color):
    global message_from_last_player
    my_color = color  # Set colors for self and opponent
    if color == "B":
        Opponent_color = "W"
    else:
        Opponent_color = "B"
    ...
    the_input = False   # Clear past input
    while the_input == False:   # Gameplay loop to search for move
        ...
        for x in range(1, 8):
            ...
            if WasWinningMove():
                ...
                message_from_last_player = (f"Player {player_ID} (AI): {player_name} plays {my_color.upper()} in column {x}")   # Added AI play reason to string for debugging
                the_input = True
                break

        #Skip this if an above higher priority move was identified
        # This block looks for blocking moves. If I cant win neither will you! muahaha
        for x in range(1, 8):
        ...
        if the_input == False:
            tries = 5   # Look 5 times for a valid move
            while True:
                if tries <= 0:
                    break       # Cut out of this while loop
                My_Move = CheckOtherMoves(sim_board, my_color)
                if int(My_Move) != -1:
                    if ValidatePiece(game_board, int(My_Move) + 1) == True:
                        AddPiece(int(My_Move)+1, my_color.upper())  # Plays move on real board
                        message_from_last_player = (f"Player {player_ID} (AI): {player_name} plays {my_color.upper()} in column {My_Move+1}")  # Added AI play reason to string for debugging
                        the_input = True
                        break
                    else:
                        tries -= 1    # Invalid move
                else:
                    tries -= 1    # function returned code to just go random.
        ...
        if ValidatePiece(game_board, AI_Guess_Random) == True and the_input == False:
            AddPiece(AI_Guess_Random, color.upper())
            message_from_last_player = (f"Player {player_ID} (AI): {player_name} plays {color.upper()} in column {AI_Guess_Random}")  # Added AI play reason to string for debugging
            the_input = True
...
def CheckOtherMoves(board, color):
    # Check horizontal for chances to make a EXXXE situation that ensures a win next turn
    # Does not check for ensuring play would land on correct row. Theory is even if it doesnt then it sets up a better move next turn
    for x in range(0, 36, 7):   #Cycle through rows starting at 0 and each multiple of 7 following
        row_string = ""         # clear out row string on each row
        for y in range(0,7):    # cycle through each column on the selected row
            row_string = row_string + board[x+y]     # Creates a left to right string of each row
            if (f"EE{color}{color}E") in row_string:    # Checks for new key conditions as the string is generated to capture the correct point to make a play.
                return (y - 3)                          # Play piece on 2nd slot in the ID'd string section
            elif (f"E{color}{color}EE") in row_string:  # Checks for new key conditions as the string is generated to capture the correct point to make a play.
                return (y - 1)                          # Play piece on next to last slot in the ID'd string section

    # ***No special horizontal checks as thats not really a clever move. Those "just add 1" moves will be below if nothing better is found first

    # Check diagonal very much like horizontal
    # Does not check for ensuring play would land on correct row. Theory is even if it doesnt then it sets up a better move next turn
    right_diag_starts = [14, 7, 0, 1, 2, 3]     # Shortest way I could think of doing this would be make a list of all starting points that have at least 4 spots in their diagonal line.
    for x in range(len(right_diag_starts)):     # Cycle through each starting point
        diag_string = ""                        # Clear out string same as others
        reached_edge = False                    # Gotta stop looking for slots at the edge of the board
        for y in range(0,10):                   # Made this big enough to cover all situations plus some. I'll weed out numbers that are too big.
            if right_diag_starts[x-1]+(y*8) <= len(board)-1 and reached_edge == False:     # Check to ensure id'd spot is within game_board list and we have not reached an edge
                diag_string = diag_string + board[right_diag_starts[x-1]+(y*8)]            #Add spot to list of id'd spots
                if ((right_diag_starts[x-1]+(y*8))+1) % 7 == 0:                                 # If spot is 1 less than a number that's divisible by 7 then its on the right edge
                    reached_edge = True         # Since this spot is on the edge we don't want to look further.
            if (f"EE{color}{color}E") in diag_string:   # Checks for new key conditions as the string is generated to capture the correct point to make a play.
                return (y - 3)                          # Play piece on 2nd slot in the ID'd string section
            elif (f"E{color}{color}EE") in diag_string: # Checks for new key conditions as the string is generated to capture the correct point to make a play.
                return (y - 1)                          # Play piece on next to last slot in the ID'd string section

    # Check diagonal exactly like right diag starts
    # Does not check for ensuring play would land on correct row. Theory is even if it doesnt then it sets up a better move next turn
    diag_string = ""
    Left_diag_starts = [3, 4, 5, 6, 13, 20]
    for x in range(len(Left_diag_starts)):
        ...
        reached_edge = False
        for y in range(0,10):
            if Left_diag_starts[x-1]+(y*6) <= len(board)-1 and reached_edge == False:
                diag_string = diag_string + board[Left_diag_starts[x-1]+(y*6)]
                if (Left_diag_starts[x-1]+(y*6)) % 7 == 0:
                    reached_edge = True
            if (f"EE{color}{color}E") in diag_string:   # Checks for new key conditions as the string is generated to capture the correct point to make a play.
                return (y - 3)                          # Play piece on 2nd slot in the ID'd string section
    ...
    # Ok now on to the boring moves if nothing cool was found
    # This is just intended to drop near exisiting moves
    for x in range(0, 36, 7):   #Cycle through rows starting at 0 and each multiple of 7 following
        row_string = ""         # clear out row string on each row
        for y in range(0,7):    # cycle through each column on the selected row
            row_string = row_string + board[x+y]     # Creates a left to right string of each row
            if (f"E{color}{color}E") in row_string:    
                my_decider = random.randint(1,2)
                if int(my_decider) == 1:
                    return (y - 3)                      
                else:
                    return y                      
            elif (f"{color}{color}") in row_string:   
                my_decider = random.randint(1,3)
                if int(my_decider) == 1:
                    return y                      
                elif int(my_decider) == 2:
                    return y + 1
                else: 
                    return y-1
            
    # no doubles so place near a single
    for x in range(0, 36, 7):   #Cycle through rows starting at 0 and each multiple of 7 following
        row_string = ""         # clear out row string on each row
        for y in range(0,7):    # cycle through each column on the selected row
            row_string = row_string + board[x+y]     # Creates a left to right string of each row
            if (f"{color}") in row_string:    
                my_decider = random.randint(1,5)    # We see this piece but there will likely be others. we cant be totally predictable so 50% chance this gets played near. If none of these trigger its a random play anyway so who cares.
                if int(my_decider) == 1:
                    return y
                elif int(my_decider) == 2:
                    return y + 1
                elif int(my_decider) == 3:
                    return y - 1
                else:
                    pass        # Look for other options

    # Ok we got all the way here meaning its probably close to turn 1 and we'll just play random.
    return -1   # Secret code (lol) for just play random
